...
class ProductRepositoryImpl @Inject constructor(
    ...
    ): Result<Pair<List<Product>, Int>> = runCatching {
    ...
    override suspend fun uploadProductImage(
        ...
        if (response.isSuccessful) {
            response.body()?.imageUrl
                ?: error("El servidor no devolvió una URL de imagen")
        } else {
...
/**
 * Convierte una URI (galería/cámara) en MultipartBody.Part para Retrofit.
 */
internal fun Uri.toMultipart(
    context: Context,
    fieldName: String
): MultipartBody.Part {
