...
class PantallaDialogs extends StatelessWidget {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(esError
            ? 'Error: no se pudo conectar al aeropuerto'
            : 'Operación aeroportuaria registrada correctamente'),
  ...
  Future<void> _mostrarConfirmacion(BuildContext context) async {
    final confirmar = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          '¿Estás seguro de que deseas eliminar Aeropuerto Mariscal Sucre?\n'
    ...
    if (confirmar == true) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Aeropuerto eliminado correctamente')),
      );
    }
  }
  ...
  Future<void> _mostrarFormulario(BuildContext context) async {
    ...
    await showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Agregar aeropuerto'),
        content: Form(
          key: formKey,
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                validator:  (v) {
                  ...
                  if (!RegExp(r'^[A-Za-z]{3}$').hasMatch(v.trim())) return 'Formato: UIO';
                  return null;
                },
              ),
            ],
    ...
    if (ctrlNombre.text.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Aeropuerto "${ctrlNombre.text}" agregado')),
      );
    }
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          OutlinedButton.icon(
            ...
            label: const Text('Eliminar aeropuerto (confirmación)'),
          ),
          const SizedBox(height: 8),
          FilledButton.tonal(
            onPressed: () => _mostrarFormulario(context),
            child: const Text('Agregar aeropuerto (formulario)'),
          ),
        ],
      ),
    );
